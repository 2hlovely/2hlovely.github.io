...
import { FileText, Github, Linkedin, Pin } from 'lucide-react';
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-wrap justify-center gap-3 sm:gap-4 mb-6 relative px-2">
                ...
                {social.cv && (
                    <button type="button" onClick={() => setShowCv(true)} className="p-2 text-neutral-600 transition-colors hover:text-accent dark:text-neutral-400" aria-label="CV">
            ...
            {researchInterests && researchInterests.length > 0 && (
                <div className="mb-6 bg-background text-center">
                    <h3 className="mb-2 font-semibold text-primary">{messages.profile.researchInterests}</h3>
                    <p className="text-sm leading-relaxed text-neutral-700 dark:text-neutral-500">
                        {researchInterests.map((interest, index) => (
                            <span key={interest}>
                                {index > 0 && ' '}
                                <span className="text-accent">•</span>{interest}
                            </span>
                        ))}
                    </p>
                </div>
            ...
            {features.enable_visitor_stats && <VisitorStats code={features.goatcounter_code} mapUrl={features.visitor_map_url} />}
            {social.cv && <CvModal open={showCv} onClose={() => setShowCv(false)} url={social.cv} />}
