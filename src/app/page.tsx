    ...
    switch (section.type) {
      ...
      case 'publications': {
        ...
        const filteredPubs = section.filter === 'selected'
          ...
          : allPubs;
        const publications = filteredPubs;
        return {
          ...section,
          publications: section.limit ? publications.slice(0, section.limit) : publications,
        };
      }
      case 'list': {
...
function loadPageDataForLocale(locale: string | undefined): HomePageLocaleData {
  ...
  const aboutConfig = getPageConfig<{
    profile?: { research_interests?: string[]; timeline?: TimelineItem[] };
    sections?: SectionConfig[];
  }>('about', locale);
  const researchInterests = aboutConfig?.profile?.research_interests;
  const timeline = aboutConfig?.profile?.timeline;
